...
router.get("/", async (req, res) => {
  try {
    ...
    const sql = showAll
      ? "SELECT id, nama, aktif FROM kategori_it ORDER BY nama ASC"
      : "SELECT id, nama, aktif FROM kategori_it WHERE aktif = 1 ORDER BY nama ASC";
...
router.put("/:id", requireRole("super_admin"), async (req, res) => {
  try {
    ...
    await pool.query("UPDATE kategori_it SET aktif = ? WHERE id = ?", [
      aktif ? 1 : 0,
      req.params.id,
    ]);
...
router.post("/", requireRole("super_admin"), async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO kategori_it (nama) VALUES (?)",
      [nama.trim()],
    );
...
router.delete("/:id", requireRole("super_admin"), async (req, res) => {
  try {
    await pool.query("DELETE FROM kategori_it WHERE id = ?", [req.params.id]);
